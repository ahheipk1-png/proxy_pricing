...
def simulate_state(spot0, month, kind, params, rng, n_paths):
    ...
    while paths_left:
        ...
        normals = np.vstack((base, -base))[:batch]
...
def fit_proxy(spots, values, kind, params, method):
    ...
    if method in {"pchip", "akima"}:
        slopes = pchip_slopes(x, target) if method == "pchip" else akima_slopes(x, target)
        curve = hermite_predictor(x, target, slopes)
    else:
    ...
    def predict(new_spots):
        ...
        fitted = np.maximum(
            np.exp(np.clip(curve(np.log(np.maximum(new_spots, 1e-12))), -30.0, 20.0))
            - LOG_EPS,
            0.0,
        )
