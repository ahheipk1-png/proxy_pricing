...
def feature_normals(params, n_paths=4096):
    rng = np.random.default_rng(params.seed + 1009)
    half = n_paths // 2
    z_var = rng.standard_normal((half, 3))
    z_market = rng.standard_normal((half, 3))
    z_var = np.vstack((z_var, -z_var))
    z_market = np.vstack((z_market, -z_market))
...
def simulate_state(
    ...
    while paths_left:
        batch = min(paths_left, SIMULATION_BATCH)
        quarter = (batch + 3) // 4
        base_var = rng.standard_normal((quarter, n_steps, 3))
        base_market = rng.standard_normal((quarter, n_steps, 3))
        ...
        shifted_count = max(batch - 2 * quarter, 0)
        if shifted_count == 0:
            likelihood = np.ones(batch)
        else:
            log_ratio = (
